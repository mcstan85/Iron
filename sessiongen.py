...
fallen = Client("MarkMusic", api_id=API_ID, api_hash=API_HASH, in_memory=True)


async def main():
    await MarkMusic.start()
    sess = await MarkMusic.export_session_string()
    txt = f"Here is your Pyrogram {ver} String Session\n\n<code>{sess}</code>\n\nDon't share it with anyone.\nDon't forget to join @marrkmusic"
    ok = await MarkMusic.send_message("me", txt)
